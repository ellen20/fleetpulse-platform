...
interface UsersFixture {
  manager: { email: string; password: string; name: string; role: string }
  viewer: { email: string; password: string; name: string; role: string }
}
...
describe('Dashboard - Header', () => {
  ...
  it('displays vehicle and driver count', () => {
    cy.get('[data-testid="status-count-all"]').invoke('text').then((vehicleCount: string) => {
...
describe('Dashboard - Stat Cards', () => {
  ...
  it('total fleet equals sum of all statuses', () => {
    cy.get('[data-testid="status-count-all"]').invoke('text').then((total: string) => {
      cy.get('[data-testid="status-count-available"]').invoke('text').then((avail: string) => {
        cy.get('[data-testid="status-count-charging"]').invoke('text').then((charging: string) => {
          cy.get('[data-testid="status-count-maintenance"]').invoke('text').then((maintenance: string) => {
...
describe('Dashboard - Fleet Map', () => {
  ...
  it('clicking Available filters to available vehicles only', () => {
    ...
    cy.get('[data-testid="status-count-available"]').invoke('text').then((count: string) => {
  ...
  it('clicking Charging filters to charging vehicles only', () => {
    cy.get('[data-testid="status-card-charging"]').click()
    cy.get('[data-testid="status-count-charging"]').invoke('text').then((count: string) => {
  ...
  it('clicking Maintenance filters to maintenance vehicles only', () => {
    ...
    cy.get('[data-testid="status-count-maintenance"]').invoke('text').then((count: string) => {
